...
The protocol has 11 business methods: `list_note_summaries`, `list_trashed_notes`, `read_note`, `read_notes` (batch read — `RemoteBackend` drives `POST /api/notes/batch-read`, raising `BatchReadUnsupportedError` when the server predates the route; `LocalBackend` loops `read_note`), `create_note`, `update_note` (returns `NoteUpdateResult` with the rename cascade's `affected_notes` list), `append_to_note`, `delete_note`, `restore_note`, `upload_attachment`, `download_attachment`, plus `close`. Shapes live in `knoten/repositories/backend.py` as frozen dataclasses — `NotesPage`, `BatchReadResult`, `NoteDraft`, `NotePatch`, `NoteUpdateResult`, `AttachmentUploadResult`, `AttachmentDownloadResult`. Services depend on the protocol, not on either implementation, so the same `edit_note_remote` / `create_note_remote` helpers drive both backends.
...
- **No permission model.** `NoteForbiddenError` cannot happen in local mode — the placeholder branches in the sync path and the rename-cascade refresh (`edit_note_remote`) are dead code for local-only users and are kept only for symmetry with `RemoteBackend`.
...
- Sync orchestration: `knoten/services/sync.py` — `incremental_sync` is the real work; `full_sync` delegates to it with a cleared cursor. The pull pass fetches note bodies in chunks of 50 via `POST /api/notes/batch-read` (`_pull_note_bodies`); when the route itself 404s (a server predating the endpoint) it falls back to per-note `GET /api/notes/{id}` for the rest of the run — `BatchReadUnsupportedError`, detected once, never retried per chunk. The reconcile catch-up, rename-cascade refresh, and post-push refetch stay per-note. Server-provided values are validated at the ingest boundary and a failure skips that one note with a warning naming it, counted in `skipped_invalid` — never aborting the run. Two cases: hostile filenames (path separators / NUL / empty) and frontmatter values containing control characters (`FrontmatterValidationError` from the writer). The same per-note skip applies to every path that mirrors a server payload: pull pass, reconcile re-fetch, post-push re-fetch, and the edit rename-cascade refresh.
- File writing: `knoten/repositories/vault_files.py` — atomic writes via tmp + rename, path derivation in `path_for_note`. The writer validates that the server-provided relative path stays inside the vault before touching disk, and rejects frontmatter values containing control characters (`FrontmatterValidationError`, enriched with the note's id + filename so interactive writes fail loudly naming the note — the mirror writes frontmatter as single-line YAML scalars, and a value embedding `\n---\n` would shift the frontmatter fence). `strip_frontmatter` lives here too — the single inverse of `render_note_markdown`, shared by services and `LocalBackend` (never re-implement it locally).
